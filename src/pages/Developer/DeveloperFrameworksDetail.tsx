...
export default function DeveloperFrameworksDetail() {
  const { id } = useParams();
  ...
  if (!project) {
    return (
      <div className="max-w-7xl mx-auto py-16 px-4" data-id="frameworks-detail-not-found">
        <p className="text-muted mb-4">Proyecto no encontrado.</p>
        <button
          onClick={() => navigate('/dev/frameworks')}
  ...
  return (
    <div className="flex flex-col" data-id="developer-frameworks-detail">
      ...
      <section data-id="detail-nav">
        <div className="max-w-7xl mx-auto pt-6 px-4 flex items-center justify-between gap-4">
          ...
          <PrevNextBtns
            onPrev={() => prev && navigate(`/dev/frameworks/${prev.id}`)}
            onNext={() => next && navigate(`/dev/frameworks/${next.id}`)}
            disabledPrev={!prev}
            disabledNext={!next}
          />
        </div>
        ...
        <section className="border-b border-border" data-id="detail-gallery">
          <div className="max-w-7xl mx-auto px-4 py-10">
            <div
              ...
              data-id="frameworks-gallery-grid"
            >
      ...
      {project.videos.length > 0 && (
        <section data-id="detail-videos">
          <div className="max-w-7xl mx-auto px-4 py-10">
            ...
            <div className="grid grid-cols-1 md:grid-cols-2 gap-6" data-id="frameworks-videos-grid">
