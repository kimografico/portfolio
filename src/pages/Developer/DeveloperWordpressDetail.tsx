...
// Extrae la URL de embed de YouTube a partir de varios formatos de URL
function getYouTubeEmbedUrl(url: string): string | null {
  // Formato largo: https://www.youtube.com/watch?v=VIDEO_ID
...
export default function DeveloperWordpressDetail() {
  const { id } = useParams();
  ...
  if (!project) {
    return (
      <div className="max-w-7xl mx-auto py-16 px-4" data-id="wp-detail-not-found">
        <p className="text-muted mb-4">Proyecto no encontrado.</p>
        <button
          onClick={() => navigate('/dev/wordpress')}
  ...
  return (
    <div className="flex flex-col" data-id="developer-wordpress-detail">
      ...
      <section data-id="detail-nav">
        <div className="max-w-7xl mx-auto pt-6 px-4 flex items-center justify-between gap-4">
          ...
          <PrevNextBtns
            onPrev={() => prev && navigate(`/dev/wordpress/${prev.id}`)}
            onNext={() => next && navigate(`/dev/wordpress/${next.id}`)}
            disabledPrev={!prev}
            disabledNext={!next}
          />
        </div>
      ...
      {project.videos.length > 0 && (
        <section data-id="detail-videos">
          <div className="max-w-7xl mx-auto px-4 py-10">
            ...
            <div className="grid grid-cols-1 md:grid-cols-2 gap-6" data-id="wp-videos-grid">
